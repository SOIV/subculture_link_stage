# Subculture / Game Event Data Platform

> 한국·일본·글로벌 서브컬처 및 게임 행사를 수집·검수·번역하여 API, 캘린더, 웹 및 외부 서비스에 제공하는 독립형 데이터 플랫폼 개발 계획서
...
- 프로젝트 상태: 기획 개편 / 구현 전
- 서비스 관계: Ayaka Project와 분리된 독립 서비스

### 0.1 v2에서 변경된 핵심 사항

- 개인용 캘린더 API에서 **독립형 행사 데이터 플랫폼**으로 범위 확대
- Ayaka Project 내부 기능이 아닌 **별도 서비스 및 별도 저장소**로 분리
- Ayaka Project의 Discord Bot은 본 서비스 API를 호출하는 클라이언트로만 동작
...
본 프로젝트는 Ayaka Project에 포함되지 않는 독립 서비스다.
...
└─ ICS 피드
          ↓ API
[Ayaka Project Discord Bot]
```

Ayaka Project의 Discord Bot은 다음 기능만 담당한다.

- 사용자 명령 수신
- 행사 API 조회
- 서버별 구독 설정 저장
- API가 제공한 데이터를 기반으로 Discord 알림 발송

크롤링, 원문 저장, 번역, 분류, 유사도 검색, 행사 데이터 관리는 행사 서비스에서 수행한다.
...
[공식 웹 / 공식 SNS / 예매처 / RSS / 수동 제보]
      ...
      [REST API]    [ICS Feed]   [Web App]   [Notification]
        ↓                                         ↓
[Ayaka Discord Bot]                     [Discord / Email 등]
```
...
### 14.4 Ayaka Project Discord Bot 연동
...
서버별 구독 설정은 Bot 또는 별도 사용자 서비스에서 저장할 수 있으나, 행사 기준 데이터는 행사 플랫폼에서만 관리한다.
...
- [ ] Ayaka Project Discord Bot API 연결
- [ ] 카테고리·작품별 ICS
- [ ] 알림 작업 생성
- [ ] Discord 알림 발송
- [ ] 임베딩 및 유사 문서 검색
...
- Ayaka Discord Bot에서 공개 API 조회 가능
...
> 본 프로젝트는 특정 Discord Bot의 부가 기능이 아니라, 한국·일본·글로벌 서브컬처 및 게임 행사 정보를 공식 출처에서 수집하고 검수·번역하여 재사용 가능한 형태로 제공하는 독립 데이터 플랫폼을 목표로 한다.
