> [!IMPORTANT]
> 이 문서는 SCLS의 과거 설계/기획 문서입니다.
> 현재 사양 및 실제 구현과 일치하지 않을 수 있으며,
> 개발 과정 기록을 위해 보존합니다.

# Subculture / Game Event Calendar API

> 공식 행사 일정 캘린더 API를 기반으로 수집·검수·번역·태그·스토리지 기능을 확장한 개발 계획서
...
본 프로젝트는 v2에서 정의한 공식 행사 일정 기준 데이터 소스를 확장한 독립형 행사 정보 서비스다. 특정 봇이나 다른 프로젝트에 소속되지 않으며, API와 ICS를 통해 여러 외부 서비스에서 재사용할 수 있도록 설계한다.
